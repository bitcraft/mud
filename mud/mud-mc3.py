...
class ChattyAI(mc.Control):
    """
    this is the prefered way to add events to a object
    """

    def register(self, target):
        super(ChattyAI, self).register(target, self.blab)

    def blab(self):
        print "something happened"


class AutoTargetAI(mc.Control):
    def __init__(self, target):
        self.observe(self.location.on_enter, self.target_victim)

    def target_victim(self, victim):
        self.target(victim)


"""
bob = mc.Mobile()
ChattyAI().register(bob)
"""
